...
export const StyledSmallerContentItem = styled(StyledContentItem)`
  display: flex;
  gap: 5px;
  color: ${({ theme }: { theme: ThemeInterface }) => theme.colors.gr2};
  font-size: ${({ theme }: { theme: ThemeInterface }) => theme.size.xs};
  flex-wrap: wrap;
`;
...
export const StyledAmount = styled.div`
  ...
  margin-top: 8px;
  margin-left: auto;
  color: ${({ theme, amount }: PropsInterface) => (amount < 0 ? theme.colors.red : amount > 0 ? theme.colors.green : theme.colors.grey)};
`;
